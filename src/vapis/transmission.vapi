...
namespace Transmission {
	...
	/**
	 * Add transmission's default settings to the {@link benc} dictionary.
	 */
	[CCode (cname = "tr_sessionGetDefaultSettings")]
	public void get_default_settings (ref variant_dict dictionary);

	/**
	 * Load settings from the configuration directory's settings.json file, using transmission's default settings as fallbacks for missing keys.
	 *
	...
	public class Session {
		...
		/**
		 * Initialize a transmission session.
		 *
		 ...
		 * @param settings transmission settings
		 */
		 ...
		 *
		 * If true, transmission will open a server socket to listen for incoming http RPC requests as described in docs/rpc-spec.txt.
		 */
		 ...
		 *
		 * The function is invoked FROM transmission'S THREAD! This means the function must be fast (to avoid blocking peers), shouldn't call transmission functions (to avoid deadlock), and shouldn't modify client-level memory without using a mutex!
		 */
	...
	public class BlockList {
		 ...
		 *
		 * @param filename The uncompressed ASCII file, or null to clear the blocklist. transmission does not keep a handle to `filename' after this call returns, so the caller is free to keep or delete `filename' as it wishes. transmission makes its own copy of the file massaged into a binary format easier to search.
	...
	public class Torrent {
		 ...
		 *
		 * This should only be used by transmission or to bootstrap a newly-instantiated object.
		 */
		public string download_dir {
		 ...
		 *
		 *
		 * NOTE: only the `tier' and `announce' fields are used. transmission derives `scrape' from `announce' and reassigns 'id'.
	...
	namespace Time {
		 ...
		 *
		 * This function returns a second counter that is updated once per second. If something blocks the transmission thread for more than a second, that counter may be thrown off, so this function is not guaranteed to always be accurate. However, it is *much* faster when 100% accuracy isn't needed.
